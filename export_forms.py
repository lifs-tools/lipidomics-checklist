...
sql = "SELECT e.fields FROM %sentries AS e INNER JOIN %sconnect_sample AS s ON e.id = s.sample_form_entry_id WHERE main_form_entry_id = ?;" % (table_prefix, table_prefix)
curr.execute(sql, (156, ))
fields = [row["fields"] for row in curr.fetchall()]
...
for _, dv in name_to_data_validation.items():
    sheet.add_data_validation(dv)
"""
dxf = DifferentialStyle(fill = PatternFill(bgColor="DDDDDD"))
r = Rule(type="expression", dxf = dxf)
r.formula = ['A3="Auto"']
sheet.conditional_formatting.add('B3', r)
"""
